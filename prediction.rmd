...
Columns with too many empty values and those not so relevant (column 1 to 7) are deleted
```{r cleanvoid}
intraining <- dataorg[, 8:160]
intraining <- intraining[, apply(intraining, 2, function(x) all(!is.na(x)))]
```

There are still `r dim(intraining)[2]` variables in the data, which is still too large dataset for running prediction. In this case, those highly correlated variables are excluded, because they can be represented by their correlated variables.
```{r cleancorr}
...
### Use Boosting and Ramdom Forest method to build the fitting model
```{r mod}
...
modegbm <- train(classe~., data = training, method = "gbm", verbose = FALSE)
...
confrf
```
In this comparison, the overall accuracy of gbm model is `r confgbm$overall["Accuracy"]*100`%, and the overall accuracy of random forest model is `r confrf$overall["Accuracy"]*100`%. So it's obvious that the rf model fits more in this study.
